...
void Optimizer::PartialBatchOptimization(Map* pMap, const cv::Mat Calib_K, const int WINDOW_SIZE)
{
    ...
    // UniLab：统计窗口内出现过的object序号和LabCount：持续了多少帧
    ...
    for (int i = StaticStartFrame; i < N; ++i)
    {
        ...
        // 遍历当前帧的静态特征点信息（函数开始前统计的），选取有用的构成边
        ...
        if (STATIC_ONLY==false)
        {
            ...
            else
            {
                ...
                for (int j = 0; j < vnFeaLabDyn[i].size(); j++)
                {
                    ...
                    // 第一次观测只添加动态点顶点和点与帧之间的观测边，否则还要添加动态点之间的运动边
                    if (PositionID==0)
                    {
...
int Optimizer::PoseOptimizationFlow2Cam(Frame *pCurFrame, Frame *pLastFrame, vector<int> &TemperalMatch)
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(mono)
        {
            ...
            // Set Binary Edges
            // 添加位姿和光流节点之间的边，误差为上一帧特征点处于当前位姿下在当前帧的投影和光流结果之间的误差
...
int Optimizer::PoseOptimizationFlow2Cam_change(Frame *pCurFrame, Frame *pLastFrame, vector<int> &TemperalMatch)
{
    float rp_thres = 0.04; // 0.01
    bool updateflow = true;
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(mono)
        {
            ...
            // 添加光流节点
            g2o::VertexSBAFlow* vFlo = new g2o::VertexSBAFlow();
            Eigen::Matrix<double,3,1> FloD = Converter::toVector3d(pLastFrame->ObtainFlowDepthCamera_change(TemperalMatch[i],0));
            vFlo->setEstimate(FloD.head(2));
            const int id = i+1;
            vFlo->setId(id);
            vFlo->setMarginalized(true);
            ...
            Eigen::Matrix<double,2,1> obs_2d;
            const cv::KeyPoint &kpUn = pLastFrame->mvStatKeyPs[TemperalMatch[i]]->mvKey;
            obs_2d << kpUn.pt.x, kpUn.pt.y;
    ...
    for (int i = 0; i < N; ++i)
    {
        ...
        {
            Eigen::Vector2d flow_new = vFlow->estimate();
            pCurFrame->mvStatKeyPsMatch[TemperalMatch[i]]->mvKey.pt.x = pLastFrame->mvStatKeyPs[TemperalMatch[i]]->mvKey.pt.x + flow_new(0);
            pCurFrame->mvStatKeyPsMatch[TemperalMatch[i]]->mvKey.pt.y = pLastFrame->mvStatKeyPs[TemperalMatch[i]]->mvKey.pt.y + flow_new(1);
            // 罪魁祸首？罪魁祸首！
            // pLastFrame->mvStatKeyPs[TemperalMatch[i]]->mvKey.pt.x = flow_new(0);
            // pLastFrame->mvStatKeyPs[TemperalMatch[i]]->mvKey.pt.y = flow_new(1);
        }
    }
    int inliers = nInitialCorrespondences-nBad;
    cout << "(Camera) inliers number/total numbers: " << inliers << "/" << nInitialCorrespondences << endl;
...
void Optimizer::PartialBatchOptimization_change(Map* pMap, const cv::Mat Calib_K, const int WINDOW_SIZE)
{
