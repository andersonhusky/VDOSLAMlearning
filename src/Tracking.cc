...
//! track camera and objects from the RGBD input
//!
//! \param imRGB  RGB img;
//! \param imD Depth img;
//! \param imFlow optical flow img;
//! \param maskSEM mask img;
//! \param mTcw_gt \param vObjPose_gt pose groundtruth of camera and objects;
//! \param timestamp
//! \param imTraj a variable to store the camera's trajectory;
//! \param nImage number of input img;
//! \return Tcw camera's pose
...
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB, const vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> &imObjPcl, const cv::Mat &imFlow,
                                const cv::Mat &maskSEM, const cv::Mat &mTcw_gt, const vector<vector<float> > &vObjPose_gt,
                                const double &timestamp, cv::Mat &imTraj, const int &nImage)
{
    // initialize some paras
    ...
    // Save map in the tracking head (new added Nov 14 2019)
    cv::Mat imDepth = cv::Mat::zeros(mImGray.size(), CV_32F);
    mDepthMap = imDepth;
    mFlowMap = imFlow;
    mSegMap = maskSEM;
    ...
    if(mState!=NO_IMAGES_YET)
    {
        ...
        mCurrentFrame.N_s = mCurrentFrame.mvStatKeys.size();
        mCurrentFrame.mvStatDepth = std::vector<float>(mCurrentFrame.N_s,-1);
        int num_without_itpD = 0, num_with_itpD = 0;
        for(int i=0; i<mCurrentFrame.N_s; i++)
        {
            const cv::KeyPoint &kp = mCurrentFrame.mvStatKeys[i];
            ...
            if (u<(mImGray.cols-1) && u>0 && v<(mImGray.rows-1) && v>0)
            {
                ...
                if(d>0){
                    ...
                    d = DepthInterpolation(maskSEM, imDepth, v, u, false);
                    if(d>0){
                        imDepth.at<float>(v,u) = d;
                        mCurrentFrame.mvStatDepth[i] = d;
                        ++num_with_itpD;
                    }
                }
        ...
        // std::cout << "num without interpolation depth: " << num_without_itpD << "; num with interpolation depth: " << num_with_itpD << std::endl;
        ...
        for (int i = 0; i < mCurrentFrame.mvObjKeys.size(); ++i)
        {
            ...
            d = d>0? d: DepthInterpolation(maskSEM, imDepth, v, u, true);
            imDepth.at<float>(v,u) =d;
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        for (int i = 0; i < ObjIdNew.size(); ++i)
        {
            ...
            // 当前object 是否有真值标志位，1：上一帧真值；2：下一帧真值
        ...
        cv::Mat CameraPoseTmpGT = Converter::toInvMatrix(mCurrentFrame.mTcw_gt);        // 相机位姿真值
        mpMap->vmCameraPose_GT.push_back(CameraPoseTmpGT);

        // (8) Ground Truth Rigid Motions
        std::vector<cv::Mat> Mot_Tmp_gt;                // 相对运动真值，第一位是相机的，其余是objectde
        ...
        std::vector<float> Speed_Tmp_gt;                // 速度真值，第一位为相机设为1，其余为object速度
        ...
        std::vector<cv::Mat> Centre_Tmp;                // 计算的质心，第一位为相机设置为0，其余为object质心
    ...
    if ( (f_id-nOVERLAP_SIZE+1)%(nWINDOW_SIZE-nOVERLAP_SIZE)==0 && f_id>=nWINDOW_SIZE-1 && bLocalBatch)
    {
        ...
        for(int i=0; i<20; ++i){
            std::cout << "Xw: " << mpMap->vp3DPointSta[3][i] << std::endl;
            cv::Mat Xw1 = Optimizer::Get3DinWorld(mpMap->vpFeatSta[3][i],mpMap->vfDepSta[3][i],mK, mpMap->vmCameraPose[3]);
            cv::Mat Xc = Optimizer::Get3DinCamera(mpMap->vpFeatSta[3][i],mpMap->vfDepSta[3][i],mK);
...
//! 函数功能 初始化，起其实就是保存一下特征点，将位姿初始化一下
//!
//! \return 无
...
void Tracking::GetSceneFlowObj()
{
    ...
    // viz_props.birdeye_scale_factor_ = 10.0;
    // viz_props.birdeye_left_plane_ = -20.0;
    // viz_props.birdeye_right_plane_ = 20.0;
    // viz_props.birdeye_far_plane_ = 80.0;
...
std::vector<std::vector<int> > Tracking::DynObjTracking()
{
    ...
    // 通过检测的点和对应label，obj_dis_tres记录是否通过
...
//! 函数功能 计算相机的初始位姿（P3P和运动模型选择较好的那个），P3P的结果只与光流结果有关
//!
...
cv::Mat Tracking::GetInitModelCam(const std::vector<int> &MatchId, std::vector<int> &MatchId_sub)
{
    ...
    // 世界坐标点、图像上对应2d点、相机内参、畸变参数、旋转（角轴形式）、平移、无初值
    // 迭代次数、离群点阈值、概率阈值、内点序列、最小子集计算模型
...
cv::Mat Tracking::GetInitModelObj(const std::vector<int> &ObjId, std::vector<int> &ObjId_sub, const int objid)
{
    ...
    // construct input
    // 根据输入的点序号构造输入
    std::vector<cv::Point2f> cur_2d(N);                 // 当前帧2d坐标向量
    std::vector<cv::Point3f> pre_3d(N);                 // 上一帧对应3d坐标向量
    for (int i = 0; i < N; ++i)
    {
        cv::Point2f tmp_2d;
    ...
    // 获取当前track的object序号，如果上一帧也有这个object，则生成该object的运动模型
...
void Tracking::DrawSparseFlowBirdeye(
        const std::vector<Eigen::Vector3d> &pts,    // 上一帧3D特征点
        const std::vector<Eigen::Vector3d> &vel,    // 场景流
        const cv::Mat &camera,                                      // 相机位姿
        const BirdEyeVizProperties &viz_props,      // 自定义类
        cv::Mat &ref_image)                                             // 鸟瞰图
{
...
//! 函数功能 更新当前帧信息
//!
//! \param TM_sta 静态计算时的内点，GetInitModelCam的结果
//! \return 返回值
void Tracking::RenewFrameInfo(const std::vector<int> &TM_sta)
{
    ...
    // use sampled or detected features 1200 / 800
    int max_num_sta = nMaxTrackPointBG;
    int max_num_obj = nMaxTrackPointOBJ;

    // 被筛选为内点的关键点，光流匹配点，光流向量，内点（该部分来自光流匹配，经过了几何检验）；
    // 同时从Frame创建的特征点中挑选补充（该部分在Frame中直接创建，没有经过几何检验）
    ...
    // Update
    // 将更新后的部分替换掉原来Frame中存储的部分，即直接构建的ORB特征点替换为内点特征点+直接构建的一部分
    ...
    std::vector<cv::KeyPoint> mvObjKeysTmp;                         //内点的object特征点
    std::vector<float> mvObjDepthTmp;                                       // 内点的object深度
    std::vector<cv::KeyPoint> mvObjCorresTmp;                           // 内点光流对应的位置
    std::vector<cv::Point2f> mvObjFlowNextTmp;                      // 内点的光流向量
    std::vector<int> vSemObjLabelTmp;                                       // 内点的在当前帧所在object序号
    std::vector<int> DynInlierIDTmp;                                            // 内点序号
    std::vector<int> vObjLabelTmp;                                              // object在整个处理过程中的序号

    // (1) Again, save the inliers from last frame
    // ①内点添加
    // ObjInlierSet每个vector对应了通过筛选的object点，对应的bObjStat记录着每组是否有用
    // 若有用则vector中每一位记录了内点的序号
    ...
    for (int i = 0; i < ObjInlierSet.size(); ++i)
    {
        ...
        for (int j = 0; j < ObjInlierSet[i].size(); ++j)
        {
            ...
            if (mSegMap.at<int>(y,x)!=0 && mDepthMap.at<float>(y,x)<mThDepthObj && mDepthMap.at<float>(y,x)>0)
            {
                ...
                if (x+flow_x < mImGrayLast.cols && y+flow_y < mImGrayLast.rows && x+flow_x>0 && y+flow_y>0)
                {
                    ...
                    // vSemObjLabelTmp.push_back(mCurrentFrame.vSemObjLabel[ObjInlierSet[i][j]]);
...
// ---------------------------------------------------------------------------------------
// ---------------------------------------------------------------------------------------
float Tracking::DepthInterpolation(const cv::Mat &maskSEM, const cv::Mat &imDepth, const int &row, const int &col, const bool dyna){
    cv::RNG rng((unsigned)time(NULL));
    int window_size = 20;
...
int Tracking::GetObjectIdx(const cv::Mat &imObjidx, const int &row, const int &col){
    ...
    int result_idx=0, second_idx=0;
    int max_num=0, second_num=0;
    for(auto p: obj_idxs){
        if(p.second>=max_num){
            second_idx = result_idx;
            result_idx = p.first;
            second_num = max_num;
            max_num = p.second;
        }
        else if(p.second>second_num){
            second_idx = p.first;
            second_num = p.second;
        }
    }
    ...
    if(result_idx==0){
        return max_num*0.7>=second_num? 0: second_idx;
    }
    return result_idx;
}

cv::Mat Tracking::GrabImageRGBD_change(const cv::Mat &imRGB, cv::Mat &imD, const cv::Mat &imFlow,
                                const cv::Mat &maskSEM, const cv::Mat &mTcw_gt, const vector<vector<float> > &vObjPose_gt,
                                const double &timestamp, cv::Mat &imTraj, const int &nImage)
{
    // initialize some paras
    ...
    mCurrentFrame = Frame(mImGray,imDepth,imFlow,maskSEM,timestamp,mpORBextractorLeft,mK,mDistCoef,mbf,mThDepth,mThDepthObj,nUseSampleFea, true);
    ...
    if(mState!=NO_IMAGES_YET)
    {
        ...
        mCurrentFrame.mvStatKeyPsMatch = std::vector<KeyP*>(mLastFrame.N_s);
        for(int i=0; i<mLastFrame.N_s; ++i){
            mCurrentFrame.mvStatKeyPsMatch[i] = new KeyP();
            const cv::KeyPoint &kp = mLastFrame.mvStatKeyPs[i]->mvCorre;
            const int v = kp.pt.y;
            const int u = kp.pt.x;

            mCurrentFrame.mvStatKeyPsMatch[i]->mvKey = kp;
            if (u<(mImGray.cols-1) && u>0 && v<(mImGray.rows-1) && v>0)
            {
                float d = imDepth.at<float>(v,u); // be careful with the order  !!!

                if(d>0)
                    mCurrentFrame.mvStatKeyPsMatch[i]->mvDepth = d;
            }
        }
        mCurrentFrame.N_s = mLastFrame.N_s;
...
void Tracking::Track_change()
{
    ...
    else
    {
        ...
        // TemperalMatch：为与上一帧特征点一对一的光流匹配"1 2 3 4 5 ..."
        // TemperalMatch_subset：为内点序号"1 3 4 6 ....""
        cv::Mat iniTcw = GetInitModelCam_change(TemperalMatch,TemperalMatch_subset);
        e_1_1 = clock();
        ...
        mCurrentFrame.SetPose(iniTcw);
        if (bJoint)
            Optimizer::PoseOptimizationFlow2Cam_change(&mCurrentFrame, &mLastFrame, TemperalMatch_subset);
        else
            Optimizer::PoseOptimizationNew(&mCurrentFrame, &mLastFrame, TemperalMatch_subset);
        // cout << "pose after update: " << endl << mCurrentFrame.mTcw << endl;
        e_1_2 = clock();
        ...
        double map_upd_time;
        s_4 = clock();
        RenewFrameInfo_change(TemperalMatch_subset);
        e_4 = clock();
        map_upd_time = (double)(e_4-s_4)/CLOCKS_PER_SEC*1000;
        all_timing[4] = map_upd_time;
        cout << "map updating time: " << map_upd_time/ObjIdNew.size() << endl;
        ...
        // (1) detected static features, corresponding depth and associations
        mpMap->vpFeatStatic.push_back(mCurrentFrame.mvStatKeyPs);
    ...
    if ( (f_id-nOVERLAP_SIZE+1)%(nWINDOW_SIZE-nOVERLAP_SIZE)==0 && f_id>=nWINDOW_SIZE-1 && bLocalBatch)
    {
        ...
        s_5 = clock();
        // Get Partial Batch Optimization
        Optimizer::PartialBatchOptimization_change(mpMap,mK,nWINDOW_SIZE);
        e_5 = clock();
        loc_ba_time = (double)(e_5-s_5)/CLOCKS_PER_SEC*1000;
        mpMap->fLBA_time.push_back(loc_ba_time);
        // cout << "local optimization time: " << loc_ba_time << endl;
...
void Tracking::Initialization_change()
{
    cout << "Initialization ......" << endl;

    {
        // static
        std::vector<KeyP*> points = mCurrentFrame.mvStatKeyPs;
        for (int i = 0; i < points.size(); ++i)
        {
            cv::Mat point_3d = Optimizer::Get3DinCamera(points[i]->mvKey, points[i]->mvDepth, mK);
            mCurrentFrame.mvStatKeyPs[i]->mv3DPoint = point_3d;
        }
        // dynamic
        for(int i=0; i<mCurrentFrame.mvObjects.size(); ++i)
        {
            std::vector<KeyP*> points = mCurrentFrame.mvObjects[i]->mvObjKeyPs;
            for(int j=0; j<points.size(); ++j)
            {
                cv::Mat point_3d = Optimizer::Get3DinCamera(points[i]->mvKey, points[i]->mvDepth, mK);
                mCurrentFrame.mvObjects[i]->mvObjKeyPs[j]->mv3DPoint = point_3d;
            }
        }
        // std::vector<cv::Mat> mvObj3DPointTmp;
        // for (int i = 0; i < mCurrentFrame.mvObjKeys.size(); ++i)
    ...
    // (1) save detected static features and corresponding depth
    mpMap->vpFeatStatic.push_back(mCurrentFrame.mvStatKeyPs);
    mpMap->vpObjDyna.push_back(mCurrentFrame.mvObjects);
    // (2) save detected dynamic object features and corresponding depth
    mpMap->vpFeatDyn.push_back(mCurrentFrame.mvObjKeys);  // modified Nov 19 2019
    mpMap->vfDepDyn.push_back(mCurrentFrame.mvObjDepth);  // modified Nov 19 2019
    mpMap->vp3DPointDyn.push_back(mCurrentFrame.mvObj3DPoint);  // modified Dec 17 2019
    ...
    mLastFrame = Frame(mCurrentFrame);  //  important !!!
    mLastFrame.mvStatKeyPs = mCurrentFrame.mvStatKeyPs;
...
cv::Mat Tracking::GetInitModelCam_change(const std::vector<int> &MatchId, std::vector<int> &MatchId_sub)
{
    // 保存最终位姿
    cv::Mat Mod = cv::Mat::eye(4,4,CV_32F);
    int N = MatchId.size();
    ...
    std::vector<cv::Point2f> cur_2d(N);
    std::vector<cv::Point3f> pre_3d(N);
    std::vector<KeyP*> StatKeyPs = mCurrentFrame.mvStatKeyPsMatch;
    for (int i = 0; i < N; ++i)
    {
        // 光流结果在当前帧的位置
        cv::Point2f tmp_2d;
        tmp_2d.x = StatKeyPs[MatchId[i]]->mvKey.pt.x;
        tmp_2d.y = StatKeyPs[MatchId[i]]->mvKey.pt.y;
        cur_2d[i] = tmp_2d;
        cv::Point3f tmp_3d;
        // 通过上一帧计算的特征点世界坐标

        cv::Mat x3D_p = mLastFrame.UnprojectStereoStat_change(MatchId[i],0);
        tmp_3d.x = x3D_p.at<float>(0);
        tmp_3d.y = x3D_p.at<float>(1);
        tmp_3d.z = x3D_p.at<float>(2);
        pre_3d[i] = tmp_3d;
...
void Tracking::RenewFrameInfo_change(const std::vector<int> &TM_sta)
{
    cout << endl << "Start Renew Frame Information......" << endl;
    // ---------------------------------------------------------------------------------------
    // ++++++++++++++++++++++++++++ Update for static features +++++++++++++++++++++++++++++++
    ...
    std::vector<KeyP*> mvKeyPsTmp;

    // ①保存静态内点
    std::vector<KeyP*> StatKeyPsMatch = mCurrentFrame.mvStatKeyPsMatch;
    for (int i = 0; i < TM_sta.size(); ++i)
    {
        if (TM_sta[i]==-1)
            continue;
        ...
        // 现在存储的内容仍然是光流的匹配结果
        int x = StatKeyPsMatch[TM_sta[i]]->mvKey.pt.x;
        int y = StatKeyPsMatch[TM_sta[i]]->mvKey.pt.y;
        ...
        if(flow_xe!=0 && flow_ye!=0)
        {
            if(StatKeyPsMatch[TM_sta[i]]->mvKey.pt.x+flow_xe < mImGrayLast.cols && StatKeyPsMatch[TM_sta[i]]->mvKey.pt.y+flow_ye < mImGrayLast.rows && StatKeyPsMatch[TM_sta[i]]->mvKey.pt.x+flow_xe>0 && StatKeyPsMatch[TM_sta[i]]->mvKey.pt.y+flow_ye>0)
            {
                // auto keyp = StatKeyPsMatch[TM_sta[i]];
                // std::cout << "mvKey: " << keyp->mvKey.pt << "; ";
                // std::cout << "mvDepth: " << keyp->mvDepth << "; ";
                // cv::Mat Xw = Optimizer::Get3DinWorld(keyp->mvKey, keyp->mvDepth, mK, Converter::toInvMatrix(mCurrentFrame.mTcw));
                // std::cout << "mv3DPoint: " << Xw << "; ";
                // cv::Mat Xc = Optimizer::Get3DinCamera(keyp->mvKey,keyp->mvDepth,mK);
                // std::cout << "Xc: " << Xc << std::endl;
                // 此处将作为内点的特征点同Frame构造的特征点添加到一起了
                cv::KeyPoint point = StatKeyPsMatch[TM_sta[i]]->mvKey;
                cv::KeyPoint corre(StatKeyPsMatch[TM_sta[i]]->mvKey.pt.x+flow_xe,StatKeyPsMatch[TM_sta[i]]->mvKey.pt.y+flow_ye,0,0,0,-1);
                cv::Point2f flow(flow_xe,flow_ye);
                KeyP* key_point = new KeyP(point, corre, flow, -1, -1, false);
                mvKeyPsTmp.push_back(key_point);
                
                StaInlierIDTmp.push_back(TM_sta[i]);
            }
        }

        if (mvKeyPsTmp.size()>max_num_sta)
            break;
    }

    // cout << "accumulate static inlier number in: " << mvKeyPsTmp.size() << endl;
    // ②将不是内点的特征点也加入到当前帧，使点达到一定数目
    int tot_num = mvKeyPsTmp.size(), start_id = 0, step = 20;
    std::vector<KeyP*> mvKeyPsTmpCheck = mvKeyPsTmp;
    std::vector<KeyP*> mvKeyPsSample;
    if (nUseSampleFea==1)
        mvKeyPsSample = mCurrentFrame.mvStatKeyPs;
    else{
        std::cout << "nUseSampleFea==0! error occur!" << std::endl;
        return;
    }
    while (tot_num<max_num_sta)
    {
        ...
        for (int i = start_id; i < mvKeyPsSample.size(); i=i+step)
        {
            cv::KeyPoint point_choose = mvKeyPsSample[i]->mvKey;
            // check if this key point is already been used
            // todo: 这里点是否使用过可以使用一个数据结构记录
            float min_dist = 100;
            bool used = false;
            for (int j = 0; j < mvKeyPsTmpCheck.size(); ++j)
            {
                cv::KeyPoint point_check = mvKeyPsTmpCheck[j]->mvKey;
                float cur_dist = std::sqrt( (point_check.pt.x-point_choose.pt.x)*(point_check.pt.x-point_choose.pt.x) + (point_check.pt.y-point_choose.pt.y)*(point_check.pt.y-point_choose.pt.y) );
                if (cur_dist<min_dist)
                    min_dist = cur_dist;
                if (min_dist<1.0)
                {
            ...
            int x = point_choose.pt.x;
            int y = point_choose.pt.y;
            ...
            if(flow_xe!=0 && flow_ye!=0)
            {
                if(point_choose.pt.x+flow_xe < mImGrayLast.cols && point_choose.pt.y+flow_ye < mImGrayLast.rows && point_choose.pt.x+flow_xe > 0 && point_choose.pt.y+flow_ye > 0)
                {
                    // auto keyp = mvKeyPsSample[i];
                    // std::cout << "mvKey: " << keyp->mvKey.pt << "; ";
                    // std::cout << "mvDepth: " << keyp->mvDepth << "; ";
                    // cv::Mat Xw = Optimizer::Get3DinWorld(keyp->mvKey, keyp->mvDepth, mK, Converter::toInvMatrix(mCurrentFrame.mTcw));
                    // std::cout << "mv3DPoint: " << Xw << "; ";
                    // cv::Mat Xc = Optimizer::Get3DinCamera(keyp->mvKey,keyp->mvDepth,mK);
                    // std::cout << "Xc: " << Xc << std::endl;
                    // mvKeyPsTmp.push_back(mvKeyPsSample[i]);
                    // mvKeyPsTmp.back()->mvCorre = cv::KeyPoint(point_choose.pt.x+flow_xe,point_choose.pt.y+flow_ye,0,0,0,-1);
                    // mvKeyPsTmp.back()->mvFlow = cv::Point2f(flow_xe,flow_ye);
                    cv::KeyPoint point = point_choose;
                    cv::KeyPoint corre(point_choose.pt.x+flow_xe,point_choose.pt.y+flow_ye,0,0,0,-1);
                    cv::Point2f flow(flow_xe,flow_ye);
                    KeyP* key_point = new KeyP(point, corre, flow, -1, -1, false);
                    mvKeyPsTmp.push_back(key_point);
    ...
    mCurrentFrame.N_s_tmp = mvKeyPsTmp.size();
    // ③记录深度值
    std::vector<float> mvDepthTmp(mCurrentFrame.N_s_tmp,-1);
    for(int i=0; i<mCurrentFrame.N_s_tmp; i++)
    {
        const cv::KeyPoint &kp = mvKeyPsTmp[i]->mvKey;
        ...
        if(d>0)
            mvKeyPsTmp[i]->mvDepth = d;
    }

    // ④根据深度和位姿生成世界坐标系下3D点
    for (int i = 0; i < mCurrentFrame.N_s_tmp; ++i)
    {
        mvKeyPsTmp[i]->mv3DPoint = Optimizer::Get3DinWorld(mvKeyPsTmp[i]->mvKey, mvKeyPsTmp[i]->mvDepth, mK, Converter::toInvMatrix(mCurrentFrame.mTcw));
    }
    ...
    // Update
    // 将更新后的部分替换掉原来Frame中存储的部分，即直接构建的ORB特征点替换为内点特征点+直接构建的一部分
    mCurrentFrame.mvStatKeyPs = mvKeyPsTmp;

    // cout << "updating STATIC features finished...... " << mvKeyPsTmp.size() << endl;
...
void Tracking::GetSceneFlowObj_change()
{
    const cv::Mat Rcw = mCurrentFrame.mTcw.rowRange(0,3).colRange(0,3);
    const cv::Mat tcw = mCurrentFrame.mTcw.rowRange(0,3).col(3);
